...
from eval.eval_utils.kitti_utils import draw_boxcube, calculate_cube_error_onlyrear, draw_projected_box3d, \
    draw_bev_box3d, Calibration, read_label,\
    calculate_depth_error, sum_list, mean_list, seperate_POS_NEG
from eval.eval_utils.vis_two_box import vis_two_box
from eval.eval_utils.crop_inside import LabelParser
...
class Evaluator:
    def __init__(self, 
        ...
        self.cropper = self._init_cropper(img_shape, roi_shape)
        
    def _init_cropper(self, img_shape:tuple, roi_shape:tuple):
        img_w, img_h = img_shape
        roi_w, roi_h = roi_shape
        return LabelParser(img_w, img_h, roi_w, roi_h)
    ...
    def evaluate(self, eval2D, eval25D, eval3D, vizGT=True, video=True, box_size_range=[32, 96], channel=1,crop_box=[]):
        ...
        for j,(objs, objs_det, imgpath) in tqdm(enumerate(zip(self.labels, self.labels_det, self.imgpaths)), total=len(self.imgpaths), unit="imgs"):
            ...
            if objs != None and len(objs) >0:
                ...
                for i, obj in enumerate(objs):
                    ...
                    # 合并类
                    if obj.type == "trailerback" or obj.type == "AIV" or obj.type == "TRUCKHEAD":
                        obj.type = "TRUCK"
                    if obj.type == "VAN":
                        obj.type = "CAR"
                    # if obj.type == "Rider":
            ...
            if (eval2D or eval3D) and (viz25D or vizGT or viz3D):
                ...
                if objs != None:
                    for i,(obj, miss_flag) in enumerate(zip(objs, miss_flags)):
                        ...
                        if viz3D:
                            # 3d box and bev
                            if obj.type == 'VAN' or obj.type == 'CAR'or obj.type == 'PD'or obj.type == 'Rider' or obj.type == 'Three'or obj.type =='TRUCK' or obj.type =='BUS'or obj.type =='SPECIALCAR' or obj.type == 'trailerback':
                ...
                if objs_det != None:
                    for i, (obj_det,false_flag) in enumerate(zip(objs_det, false_flags)):   
                        ...
                        if obj_det.type == 'VAN' or obj_det.type == 'CAR'or obj_det.type == 'PD'or obj_det.type == 'Rider' or obj_det.type == 'Three'or obj_det.type =='TRUCK' or obj_det.type =='BUS'or obj_det.type =='SPECIALCAR' or obj_det.type == 'trailerback':
                            if viz3D:
                                ...
                                # 2.5D box viz
                            # if viz25D:
                            #     kp_downs = obj_det.keypoint_down.reshape((4, 2))
                            #     # print(kp_downs)
                            #     keypoint = np.zeros((4, 2), dtype=np.float32)
                            #     keypoint_vis = np.zeros((4, ), dtype=bool)
                                
                            #     for i, kp in enumerate(kp_downs):
                            #         keypoint[i, :] = kp
                            #         keypoint_vis[i] = (kp!=[-1., -1.]).any()
                                
                                
                            #     # 2.5D box
                            #     if obj_det.type != 'PD' and obj_det.type != 'Rider':
                            #         img = show_result_keypoints(img, keypoint_vis, keypoint, obj_det.box2d)
                            #     box_center = (obj_det.box2d[0:2] + obj_det.box2d[2:4]) / 2
                            #     box_center = box_center.astype(int)
                            #     # cv2.circle(img, tuple(center_proj[k]),8,(0,0,255),4)
                            #     cv2.circle(img, tuple(box_center),8,(255,255,0),2)
                            #     cv2.putText(img, '{:.3f}'.format(obj_det.alpha), tuple(box_center), cv2.FONT_HERSHEY_SIMPLEX, 
                            #                         3, (255, 0, 0), 2, cv2.LINE_AA)
                            #     cv2.rectangle(img, tuple(obj_det.box2d[0:2].astype(int)), tuple(obj_det.box2d[2:4].astype(int)), (0, 0, 255), thickness = 4)
                        ...
                        if viz25D:
                            ...
                            img_2_3 = cv2.rectangle(img_2_3, (int(obj_det.box2d[0]), int(obj_det.box2d[1])),
                                                (int(obj_det.box2d[2]), int(obj_det.box2d[3])), color=color_det,
                                                thickness=3)
        ...
        if cal_metrics:
            ...
            if eval2D:    
                ...
                parse_metrics(box_tp_list,box_fn_list, box_pre_iou_list, box_pre_num_list, box_gt_num_list, box_error_list, 
                            box_size_range, self.save_dir, name="eval_2D_results.xls")
...
def setup_eval_args():
    ...
    parser.add_argument("--box_size_range", type=int, default=[32, 96], nargs="+", help="box size range")
...
if __name__ == "__main__":
    ...
    evaluator = Evaluator(test_file, save_dir, gt_path, det_path, 
                          img_shape, roi_shape, calib_path)
    evaluator.evaluate(eval2D, eval25D, eval3D, vizGT, video, box_size_range, channel)
...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        edge_protect = True
        if edge_protect:
            X_MIN, X_MAX, Y_MIN, Y_MAX = 0, 3840, 0, 2160
        ...
        # self.mask = np.int64(self.orientation_x>0)
...
def draw_boxcube(image,obj):
    obj_x = obj.orientation_x[obj.mask]
    obj_y = obj.orientation_y[obj.mask]
    for i, (x, y) in enumerate(zip(obj_x, obj_y)):
        cv2.circle(image, (int(x), int(y)), 3, (0, 255, 0), 5)
    return image

def draw_projected_box3d(image, qs, id, color=None, cls=None, thickness=1, draw_orientation=False, draw_height_text=False,
            ...
            if obj.id != 0 and obj_det.id != 0 and obj.id == obj_det.id:
                ...
                if (obj.mask == [False,True,False,True]).all() and (obj_det.mask == [False,True,False,True]).all():#orintation判断正确且不是全样本
                    if (obj.mask==obj.inside).all():#都是false true false true
                        ...
                        obj.cal = True
                        obj_det.cal = True
                        rear_abs=(obj_det.orientation_x[3]-obj_det.orientation_x[1])-(obj.orientation_x[3]-obj.orientation_x[1])
                        rear_rlt=rear_abs/(obj.orientation_x[3]-obj.orientation_x[1])
...
def mean_list_cube(list):
    new_list = [[] for i in range(4)]
    for j in range(4):
        if len(list[j])==0:
            pass
        else:
            new_list[j].append(np.round(sum(list[j])/len(list[j]),3))
    return new_list
...
def seperate_POS_NEG(datas):
    ...
    for i,data in enumerate(datas):
        if data>=0:
            ...
            neg=neg+1
    return pos,sum_pos/pos,neg,sum_neg/neg
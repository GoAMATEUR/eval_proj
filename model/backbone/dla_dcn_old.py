...
from model.backbone.hourglass import HourglassNet
from model.backbone.hourglass import Bottleneck as btnk
from model.backbone.hourglass_firstconv import HourglassNet as hgn_firstconv
from model.backbone.hourglass_firstconv import Bottleneck as btnk_firstconv
BN_MOMENTUM = 0.1

def build_backbone(cfg):
    print("Backbone: {}".format(cfg.MODEL.BACKBONE.CONV_BODY))
    ...
    if cfg.MODEL.BACKBONE.CONV_BODY == "hg":
        ...
        model = HourglassNet(btnk,num_stacks=cfg.MODEL.BACKBONE.STACKS, num_blocks=cfg.MODEL.BACKBONE.BLOCKS)
        
    if cfg.MODEL.BACKBONE.CONV_BODY == "hg_firstconv":
        
        model = hgn_firstconv(btnk_firstconv,num_stacks=cfg.MODEL.BACKBONE.STACKS, num_blocks=cfg.MODEL.BACKBONE.BLOCKS)
    return model
